      ...
      >
        ...
        */}
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          /* Planet logo cards base styles (Neo-Brutalist) */
          .orbit-logo-card {
            position: absolute;
            left: 50%;
            top: 50%;
        ...
        >
          ...
          */}
          ...
          >
            {/* Glowing aura */}
            <div className="absolute w-[90px] h-[90px] md:w-[120px] md:h-[120px] rounded-full filter blur-md animate-custom-sun-pulse z-10 bg-[#ccff00]/60" />
            
            {/* Sun Core Logo Render */}
            {centerLogo ? (
              typeof centerLogo === "string" ? (
                <img
                  className="w-14 h-14 md:w-20 md:h-20 rounded-full border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-20 bg-white p-2 md:p-3 relative object-contain"
                  src={centerLogo}
                  alt={centerLogoAlt}
                  width={80}
                  height={80}
                ...
                <div className="w-14 h-14 md:w-20 md:h-20 rounded-full border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-20 bg-white flex items-center justify-center p-2 relative">
                  {centerLogo}
                </div>
              )
            ) : (
              <div className="w-14 h-14 md:w-20 md:h-20 rounded-full border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-20 bg-[#ccff00] flex items-center justify-center p-2 relative">
                <OrbitIcon className="w-8 h-8 text-black animate-spin" style={{ animationDuration: '10s' }} />
              </div>
            )}

            {/* Sun core dash rings */}
            <div className="absolute w-[110px] h-[110px] md:w-[140px] md:h-[140px] rounded-full border-2 border-dashed border-black/40 animate-custom-spin-cw pointer-events-none" />
            <div className="absolute w-[150px] h-[150px] md:w-[185px] md:h-[185px] rounded-full border-2 border-dashed border-black/25 animate-custom-spin-ccw pointer-events-none" />
          </div>
          ...
          {orbits.map((orbit) => {
            return (
              <React.Fragment key={orbit.id}>
                ...
                <div
                  className="absolute rounded-full border-2 border-dashed border-black/30 pointer-events-none"
                  style={{
